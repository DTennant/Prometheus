...
app = typer.Typer(
    name="openharness-evolve",
    help="Self-Bootstrapping Agent Harness: let models evolve their own optimal harness.",
...
def run(
    ...
    from openharness_evolve.config.experiment_config import ExperimentConfig, ModelConfig
    ...
    from openharness_evolve.eval.tasks import get_task_suite
...
def compare(
    ...
    from openharness_evolve.analysis.compare import compare_runs, format_comparison
...
def show(
    ...
    from openharness_evolve.analysis.compare import load_run
...
def _build_agent_client(api_format: str, api_key: str, base_url: str | None, model: str):
    ...
    # For now, return a placeholder — real API integration requires the full
    # OpenHarness engine or a direct API wrapper, which will be wired in Phase 4
    typer.echo("WARNING: Real API agent client not yet implemented, using dry-run", err=True)
...
def _build_llm_client(api_format: str, api_key: str, base_url: str | None, model: str):
    ...
    typer.echo("WARNING: Real LLM client not yet implemented, using dry-run", err=True)
